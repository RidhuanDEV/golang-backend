...
func (s *Server) mountUsers() {
	...
	register[IDInput, User](s, "user.get", func(ctx context.Context, input *IDInput, _ *Actor) (Success[User], error) {
		user, err := s.Users.Get(ctx, input.ID)
		return ok(user), err
	})
	register[UserInput, User](s, "user.create", func(ctx context.Context, input *UserInput, actor *Actor) (Success[User], error) {
		b := input.Body
		user, err := s.Users.Create(ctx, s.policy(ctx, "user.create"), actor, b)
		return ok(user), err
	})
	register[UpdateUserInput, User](s, "user.update", func(ctx context.Context, input *UpdateUserInput, actor *Actor) (Success[User], error) {
		b := input.Body
